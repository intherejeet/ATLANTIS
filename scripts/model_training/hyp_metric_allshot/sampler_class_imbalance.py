...
#     actual_size = min(len(input_data), size)  # Choose the minimum of size or input_data length
#     return np.random.choice(input_data, size=actual_size, replace=replace).tolist()

def safe_random_choice(input_data, size):  
    if len(input_data)==2:  
        replace = False  
        return np.random.choice(input_data, size=2, replace=replace).tolist()  
    else:  
        replace = len(input_data) < size  
...
class UniqueClassSempler(Sampler):
    ...
    def __iter__(self):  
        ...
        for label_idx in idx:  
            t = self.labels_to_indices[self.labels[label_idx]]  
            # m_per_class can be larger, equal, or smaller than the number of images in class  

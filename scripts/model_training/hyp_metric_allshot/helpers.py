...
def evaluate_class_wise(get_emb_f, ds_name, hyp_c):
    if ds_name != "Inshop":
        emb_head = get_emb_f(ds_type="eval")
        emb_head_train = get_emb_f(ds_type="train")
        emb_body = get_emb_f(ds_type="eval", skip_head=True)
        recall_head = get_recall(*emb_head, ds_name, hyp_c)
        class_wise_recall_head = get_class_wise_recall(*emb_head, ds_name, hyp_c)
        train_class_wise_recall_head = get_class_wise_recall(
            *emb_head_train, ds_name, hyp_c
        )
        recall_body = get_recall(*emb_body, ds_name, 0)
    else:
        emb_head_query = get_emb_f(ds_type="query")
    ...
    return (
        recall_head,
        recall_body,
        class_wise_recall_head,
        train_class_wise_recall_head,
    )
...
def get_class_wise_recall(x, y, ds_name, hyp_c):
    if ds_name == "CUB" or ds_name == "Cars":
        k_list = [1]
    elif ds_name == "SOP":
        k_list = [1]
    ...
    for c in unique_classes:
        ...
        y_class = y[indices]
        y_cur_class = y_cur[indices]

        recall = [calc_recall_at_k(y_class, y_cur_class, k) for k in k_list]
        classwise_recall[int(c)] = recall[0]

    # print(classwise_recall)
...
def get_emb(
    ...
    path = train_path if ds_type == "train" else test_path

...
class UniqueClassSempler(Sampler):
    def __init__(self, labels, m_per_class, extra_classes= [], extra_samples=10, rank=0, world_size=1, seed=0):
    ...
    def __iter__(self):
        ...
        for i in idx:
            ...
            if label in self.extra_classes:
                idx_list += safe_random_choice(t, self.m_per_class + self.extra_samples)
            else:
